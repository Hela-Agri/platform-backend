...
class ServiceAPIController extends AppBaseController
{
    ...
    {
        $services = $this->serviceRepository->paginate($request->get('limit', 50));
    ...
    public function update($id, UpdateServiceAPIRequest $request): JsonResponse
    {
        ...
        if($service){//update the rate card name
           
            RateCard::where('item_id', $service->id)->where('item_type', 'service')
            ->update([
                'name' =>  $service->name
                ]);
